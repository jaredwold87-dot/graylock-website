...
**Alternative for NEW heroes — skip object-cover entirely (used on the realtor landing hero).** Give the section a solid bg matching the image's edge color (sample it: `magick SRC -format "%[pixel:p{0,0}]" info:`), then place the device PNG as a normal `<img>` anchored `absolute bottom-0 right-0 h-[90%] w-auto max-w-none` inside a max-width canvas, with the copy column width-capped (`w-[min(52%,620px)]`) and a left gradient ≤0.18. No crop regime exists, devices are always fully visible at every width, and no source recomposition is needed. Prefer this over full-bleed object-cover when building a hero from scratch; the recompose techniques below are for retrofitting an existing full-bleed hero.
