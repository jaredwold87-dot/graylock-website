...
**Suspense fallback gotcha (fixed Aug 2026):** pages are lazy-loaded; the route-level `<Suspense fallback>` in `App.tsx` painted `bg-navy` (#152847 blue) full-screen for a beat on every navigation until the page chunk loaded. Any full-screen fallback/skeleton must use literal `bg-[#0F0F0F]`, never navy/charcoal tokens (App.tsx is outside `theme-black`).
