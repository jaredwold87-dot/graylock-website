...
/**
 * The single mobile sticky CTA (spec §5) — one action, appears once the
 * visitor has scrolled past the hero so it never competes with the hero CTA.
 * No other competing mobile actions are exposed.
 */
export function CabinetMakerStickyCTA() {
  ...
  useEffect(() => {
    document.body.classList.toggle("cabinet-sticky-visible", visible);
    return () => document.body.classList.remove("cabinet-sticky-visible");
  }, [visible]);
  ...
  return (
    ...
    >
      ...
      >
        <CTAButton
          href={cabinetMakerGetStartedHref("sticky_cta")}
          variant="funnel"
