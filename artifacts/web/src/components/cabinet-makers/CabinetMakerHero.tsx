...
import heroDesktop from "@/assets/cabinet-hero-devices.webp";
...
const HERO_ALT =
  "A custom cabinet-maker website concept shown on a laptop and phone — premium kitchen photography, a location and service hook, and a clear consultation call to action";
...
function HeroCopy() {
  return (
    <>
      ...
      {/* H1 — second line in Graylock orange (spec). Cap tuned so each phrase
          holds a single line beside the device composition on desktop. */}
      <h1 className="font-display uppercase text-white leading-[1.05] mb-6 text-[clamp(34px,3.1vw,46px)]">
        ...
        <span className="block text-balance text-[#E85D26]">Your Website Should Be Too.</span>
      </h1>
      <p className="text-stone text-lg font-sans leading-relaxed mb-7 max-w-[540px]">
        We build custom cabinet-maker websites that showcase the quality of your work, earn
        trust before the first consultation, and turn project interest into real design and
        quote requests.
      </p>
    </>
  );
...
/**
 * Hero (spec §4.1): copy left, real device composition right, CTA above the
 * fold, proof items in a 2×2 grid on mobile.
 *
 * Desktop (1280px+) uses the supplied full-scene hero image — dark
 * architectural background with topo texture and the sample cabinetry site
 * on a laptop + phone — as a full-bleed background with copy over its empty
 * left side. Below 1280px the transparent devices cutout sits between the
 * copy and the CTA (same pattern as the well-driller hero).
 */
  ...
  return (
    <section className="relative overflow-hidden" style={{ backgroundColor: "#0f0f0f" }}>
      ...
      <div className="xl:hidden relative">
        ...
        />
        {/* Layer 2: dark walnut-grain material detail */}
        <img
          src={walnutTexture}
          alt=""
          aria-hidden="true"
          decoding="async"
          className="absolute inset-y-0 right-0 w-full h-full object-cover opacity-[0.14] pointer-events-none"
          style={{
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-12 pt-[96px] md:pt-[108px] pb-10">
          <ScrollReveal>
            <HeroCopy />
            {/* Transparent laptop + phone cutout (same asset as "What We Do") */}
            <img
              src={devicesCutout}
              alt={HERO_ALT}
        ...
        {/* Proof row — 2×2 grid on mobile (spec responsive table) */}
